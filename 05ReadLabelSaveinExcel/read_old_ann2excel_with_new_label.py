...
'''
将用旧标注方式标注的ann文件，转化为新标注方式,将其中title，ID,label信息保存在excel中
采用了新版的label
'''
...
def formatting_grade_info(grade_info):
    ...
    if '幼儿园' in grade_info:
        grade_formatting = '幼儿园'
    if '小学' in grade_info:
        grade_formatting = '小学'
    if '初中' in grade_info or '中学' in grade_info:
        grade_formatting = '初中'

    if '一年级' in grade_info or '一年' in grade_info:
        grade_formatting = '一年级'
    if '二年级' in grade_info or '二年' in grade_info:
        grade_formatting = '二年级'
    if '三年级' in grade_info or '三年' in grade_info or '三' in grade_info:
        grade_formatting = '三年级'
    if '四年级' in grade_info or '四年' in grade_info or '四' in grade_info:
        grade_formatting = '四年级'
    if '五年级' in grade_info or '五年' in grade_info:
        grade_formatting = '五年级'
    if '六年级' in grade_info or '六年' in grade_info:
        grade_formatting = '六年级'
    if grade_info == '七' or '七年级' in grade_info or grade_info == '7' or '初一' in grade_info:
        grade_formatting = '初一'
    if grade_info == '八' or '八年级' in grade_info or grade_info == '8' or '初二' in grade_info:
        grade_formatting = '初二'
    if grade_info == '九' or '九年级' in grade_info or grade_info == '9' or '初三' in grade_info:
        grade_formatting = '初三'
    if '高一' in grade_info:
        grade_formatting = '高一'
    if '高二' in grade_info:
...
def convert_label2value(label_list):
    ...
    for i in range(len(label_list)):
        ...
        if True:
            ...
            if label == 'label5' or label == 'label7':
                label_val[1].append('违反课外纪律行为')

            if label == 'label8':  # 不良行为/
            ...
            if label == 'label16' or label == 'label17' or label == 'label18':  # 情绪问题/
                label_val[4].append('抑郁问题')
            if label == 'label19' or label == 'label20' or label == 'label21':
            ...
            if label == 'label73' or label == 'label74':
                label_val[24].append('自尊心受挫')
            if label == 'label76' or label == 'label77':
                label_val[24].append('缺乏自信')
            if label == 'label78':
                label_val[24].append('认知需求不平衡')
            if label == 'label79' or label == 'label80':
