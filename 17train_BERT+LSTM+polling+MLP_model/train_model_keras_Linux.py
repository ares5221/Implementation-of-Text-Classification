...
def train_model_keras(train_data, train_label, classes_num, index, max_word_num, max_len):
    ...
    # train_label = np.reshape(train_label, (train_label.shape[0], 1, train_label.shape[1]))
    ...
    with open('./data_LSTM/' +  str(index) + 'index_bert_dict.json', 'r', encoding='utf-8') as f:
        ...
        for key in index_to_bert_vector:
            indexss = int(key)
            if indexss == max_words:
                embedding_matrix[0] = np.array(index_to_bert_vector.get(key))
            else:
                embedding_matrix[indexss] = np.array(index_to_bert_vector.get(key))
    ...
    # model.add(keras.layers.LSTM(128, return_sequences=True,input_shape=(1,768)))#如果为True的话，只返回最后一个状态的输出，是一个（samples，output_dim）2D张量，如果是False，则是返回所有序列状态的输出，是一个（samples，timesteps，output_dim）3D张量
    ...
    for index in option:
        ...
        if True:
            ...
            with open('./result/' + "keras_acc.csv", "a", encoding='utf-8', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([index, keras_sum/(exp_num*1.0)])