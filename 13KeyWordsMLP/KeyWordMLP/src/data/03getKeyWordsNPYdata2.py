...
def getLabelTestNPYData(data,name):
    ...
    if len(data) == 4 and name == 'label_attack_words':
        ...
        np.save(name + '_' + str(114) + '.npy', label_attack)
    if len(data) == 5 and name == 'label_learn_words':
        ...
        np.save(name + '_' + str(145) + '.npy', label_learn)

def get_index_keywords_json(word_list, name):
    print(word_list)
    index_2_keyword = {}
    count = 0
    for item in word_list:
        index_2_keyword[count] = item
        count +=1

    index_2_keyword_name = name + '_index_2_keyword.json'
    with open(index_2_keyword_name, 'w', encoding='utf-8') as f:
        json.dump(index_2_keyword, f, ensure_ascii=False)
...
if __name__ == '__main__':
    ...
    # 获取每个词及对应的索引id，并存储json
    get_index_keywords_json(all_attack_words, 'attack_words')
    get_index_keywords_json(all_learn_words, 'learn_words')
