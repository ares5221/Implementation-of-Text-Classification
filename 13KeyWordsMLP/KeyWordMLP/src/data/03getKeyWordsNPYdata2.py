...
def getLabelTestNPYData(data,name):
    '''生成分类的标签信息'''
    print(len(data))
    if len(data) == 3 and name == 'label_attack_words':
        print(len(data[0]), len(data[1]), len(data[2]))
        label_attack = [0 for i in range(len(data[0]))] +[1 for i in range(len(data[1]))] +[2 for i in range(len(data[2]))]
        np.save(name + '_' + str(77) + '.npy', label_attack)
    if len(data) == 4 and name == 'label_learn_words':
        ...
        np.save(name + '_' + str(101) + '.npy', label_learn)


# Start Position--->>>>>>>>>
if __name__ == '__main__':
    all_attack_words = a1.split(',') + a2.split(',') + a3.split(',')
    all_learn_words = l1.split(',') + l2.split(',') + l3.split(',') + l4.split(',')
    # getTestNPYData(all_attack_words, 'attack_words')
    getLabelTestNPYData([a1.split(','), a2.split(','), a3.split(',')], 'label_attack_words')

    # getTestNPYData(all_learn_words, 'learn_words')
    getLabelTestNPYData([l1.split(','), l2.split(','), l3.split(','), l4.split(',')], 'label_learn_words')